...
import com.trinh.japanese.view.dialog.DialogAudience;
import com.trinh.japanese.view.dialog.DialogCall;
import com.trinh.japanese.view.dialog.DialogConfirm;
import com.trinh.japanese.view.dialog.DialogStop;
...
public class M002_MainAct_Game extends AppCompatActivity implements View.OnClickListener {
    ...
    private void initData() {
        ...
        isStart = true;
        selected = 0;
        startAnim(tvContentQuestion, R.anim.anim_alpha);
        setDefaultCase(tvCaseA, R.anim.anim_right);
        setDefaultCase(tvCaseB, R.anim.anim_left);
        setDefaultCase(tvCaseC, R.anim.anim_right);
        setDefaultCase(tvCaseD, R.anim.anim_left);
        setEvent();
        countTimer();
        DataManager.getInstance().getQuestionById(new DataManager.OnDataCallBack() {
            @Override
            public void callBack(Object data) {
                M002_MainAct_Game.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
    ...
    private void setDefaultCase(TextView tv, int anim) {
        tv.setBackgroundResource(R.drawable.bg_blue_corner_30);
        tv.setVisibility(View.VISIBLE);
        startAnim(tv, anim);
    }

    public void startAnim(TextView tv, int mAnim) {
        Animation animation = AnimationUtils.loadAnimation(this, mAnim);
            ...
            new Handler().postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    for (int i = 0; i < listCase.size(); i++) {
                        if (i + 1 == trueCase) {
                            listCase.get(i + 1 < 4 ? i + 1 : (i + 1) - 4).setText("");
                            listCase.get(i + 3 < 4 ? i + 3 : (i + 3) - 4).setText("");
                            return;
                        }
                    }
                }
    ...
    private void showDialogHelpCall() {
        ...
        if (imvHelpCall.getDrawable().getLevel() == 0) {
            ...
            DialogCall callDialog = new DialogCall(trueCase, this, (key, data) -> M002_MainAct_Game.this.startTimer());
            callDialog.show();
        } else {
            return;
        }
    ...
    private void saveScoreDialog(int gravity) {
        ...
        Button btSave = dialog.findViewById(R.id.bt_save_score);
        ...
        btGetout.setOnClickListener(view -> {
            dialog.dismiss();
            finish();
        });
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                if (selected == trueCase) {
                    ...
                    M002_MainAct_Game.this.nextQuestion(selected);
                } else {
                    textView.setBackgroundResource(R.drawable.bg_red_corner_30);
                    M002_MainAct_Game.this.gameOver();
                }
            }
        }, 1500);
    }
    ...
    private void nextQuestion(int selected) {
        if (level == 15 && selected == trueCase) {
            DialogConfirm confirmDL = new DialogConfirm("Bạn đã chiến thắng !", "Tiếp tục chơi nhé"
                    , M002_MainAct_Game.this, new OnActionCallback() {
                @Override
                public void callBack(int key, Object data) {
                    if (key == DialogConfirm.KEY_CONFIRM) {
                        M002_MainAct_Game.this.saveScoreDialog(Gravity.CENTER);
                    }
                }
            });
            confirmDL.show();
            ...
            new Handler().postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    M002_MainAct_Game.this.initData();
                }
            }, 1000);
        }
    }
    ...
    private void gameOver() {
        new Handler().postDelayed(() -> {
            ...
            DialogConfirm confirm = new DialogConfirm("Game over !", "Cùng chơi tiếp nhé"
                    , M002_MainAct_Game.this, (key, data) -> {
                if (key == DialogConfirm.KEY_CONFIRM) {
                    finish();
                }
            });
